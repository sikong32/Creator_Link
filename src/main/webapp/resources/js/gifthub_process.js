...
function updateCategory() {
    ...
    if (bigCategory && category[bigCategory]) { // bigCategory의 변수 값과 category[bigCategory] 값이 존재 할 때 실행
...
function gift_add() {
	if (check()) { // if문은 true일 때 실행 됨 (즉, check 함수의 값이 true일 때 실행되는 조건)
		...
		// 선물 정보에 넣을 요소 추가 (중분류, 박스 크기, 상세품목, 상품 금액) //
		gift_info(mid_category, 'mid_category', gift_list);
		gift_info(box_size, 'box_size', gift_list);
		gift_info(detail_category, 'detail_category', gift_list);
		gift_info(gift_price, 'gift_price', gift_list);
...
function gift_info(text, classname, parent){
	var div = document.createElement('div');
	div.textContent = text;
	div.className = classname; // css 설정 할 때 필요할 것 같아서 넣음
	parent.appendChild(div);
}

function init() {
	document.getElementById('big_category').value = '';
	document.getElementById('mid_category').value = '';
